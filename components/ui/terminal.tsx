...
export interface TerminalProps {
  ...
  initialDelay?: number;
  enableSound?: boolean;
  /** When true, the typing animation starts on mount without waiting for scroll into view. */
  immediateStart?: boolean;
  /** Called once when every command has finished (including outputs). */
  onComplete?: () => void;
}
...
export function Terminal({
  ...
  const effectiveInView = immediateStart || inView;
  ...
  useEffect(() => {
    if (!effectiveInView || phase !== "idle") return;
    const t = setTimeout(() => setPhase("typing"), initialDelay);
    return () => clearTimeout(t);
  }, [effectiveInView, phase, initialDelay]);

  useEffect(() => {
    if (phase !== "done" || !onComplete || completeFired.current) return;
